...
def add_obv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["OBV_slope"] = (
        (df["OBV"] - obv_ma.shift(10)) /
        obv_ma.shift(10).abs().replace(0, np.nan)
    )
...
def detect_market_regime(
    ...
    slope_short = (sma_short - sma_short.shift(slope_window)) / sma_short.shift(slope_window)
    slope_long = (sma_long - sma_long.shift(slope_window)) / sma_long.shift(slope_window)
    ...
    # regime type
    conditions = [
        (sma_short > sma_long) & (slope_short > 0) & (slope_long > 0) & (df["regime_score"] > 0.3), # 상승장 (bull market)
        (sma_short < sma_long) & (slope_short < 0) & (slope_long < 0) & (df["regime_score"] < -0.3), # 하락장 (bear market)
